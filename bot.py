...
def check_and_update_cache(file_path, cache_name, max_age_days=1):
    if os.path.exists(file_path):
        ...
        if (datetime.now() - mod_time).days > max_age_days:
            logger.info(f"Cache {cache_name} desatualizado (>{max_age_days} dia). Limpando.")
            return {}
...
async def process_result(result, page_cache, materia_cache):
    ...
    if props is None:
        logger.error(f"Resultado inválido do Notion: 'properties' é None para {result.get('id', 'ID desconhecido')}")
        return {
            "Professor": "",
            "Status": "",
            "Tipo": "",
...
def gerar_mensagem_tarefa(tarefa):
    ...
    dias_texto = "🚨 HOJE 🚨" if dias_restantes == 0 else f"{dias_restantes} DIA{'S' if dias_restantes > 1 else ''}"
...
def delete_previous_message(chat_id, message_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Erro ao apagar mensagem: {response.status_code} - {response.text}")
...
def enviar_mensagem_telegram(mensagem, t_chat_id=TELEGRAM_CHAT_ID, parse_mode=None):
    ...
    try:
        ...
        logger.error(f"chat id {t_chat_id} - Erro ao enviar mensagem: {response.status_code} - {response.text}")
        return None
    except requests.RequestException as e:
...
async def main():
    ...
    if not all([NOTION_API_KEY, NOTION_DATABASE_ID]):
        logger.error("Variáveis de ambiente 'NOTION_API_KEY' e 'NOTION_DATABASE_ID' não definidas!")
    ...
    materia_cache = check_and_update_cache(MATERIA_CACHE_FILE, "materia_cache", max_age_days=3)
    ...
    for row in all_rows:
        ...
        if status != "Concluído" and dias_restantes is not None and 0 <= dias_restantes <= 7:
    ...
    if not all([TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, TELEGRAM_CHAT_ID_WPP]):
        raise ValueError("Variáveis 'TELEGRAM_BOT_TOKEN', 'TELEGRAM_CHAT_ID' e 'TELEGRAM_CHAT_ID_WPP' não definidas!")
    ...
    if mensagens_validas:
        ...
        if last_message_info.get("date") == current_date and "message_id" in last_message_info:
            delete_previous_message(TELEGRAM_CHAT_ID, last_message_info["message_id"])

        message_id = enviar_mensagem_telegram(mensagem_conjunta, parse_mode="MarkdownV2")
        if message_id:
        ...
        enviar_mensagem_telegram(mensagem_wpp_bc, TELEGRAM_CHAT_ID_WPP, parse_mode="Markdown")
        logger.debug(print_whatsapp_markdown(mensagem_conjunta))
        logger.info("Mensagem compatível com WhatsApp enviada!")
    else:
        logger.info("Nenhuma tarefa com Dias Restantes entre 0 e 7 e Status diferente de 'Concluído' encontrada.")
